...
public class ListarGrillaPorFechaSP extends StoredProcedure {
	ArrayList<BeanControldeProduccion> listgrilla = new ArrayList<BeanControldeProduccion>();
	String sql = Constantes.PAQUETE_CONTROLDEPRODUCCION+Constantes.LIS_GRID_POR_FECHA;
	
	public ListarGrillaPorFechaSP(DataSource ds){
		setDataSource(ds);
		setSql(sql);
