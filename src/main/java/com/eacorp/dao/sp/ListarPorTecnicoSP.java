...
public class ListarPorTecnicoSP extends StoredProcedure{
	ArrayList<BeanControldeProduccion> listgrilla = new ArrayList<BeanControldeProduccion>();
	String sql = Constantes.PAQUETE_CONTROLDEPRODUCCION+Constantes.LIS_POR_TECNICO;
	
	public ListarPorTecnicoSP(DataSource ds){
		setDataSource(ds);
		setSql(sql);
		declareParameter(new SqlParameter("codsucursal", OracleTypes.VARCHAR));
		declareParameter(new SqlParameter("codnumtecnico", OracleTypes.NUMBER));
